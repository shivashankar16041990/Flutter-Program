...
class SIFormstate extends State<SIForm> {
  ...
  Widget build(BuildContext context) {
    ...
    return Form(child :Padding(padding: EdgeInsets.all(50), key:form_key,child:
      ...
      Padding(
          ...
          child: TextFormField(
            validator: ( value)
            {
              ...
              debugPrint("from validator");
              if(value!=null){
                debugPrint("from validator null check");
              if(value.isEmpty) {

                debugPrint("from validator is empty");
              ...
              onChanged: (String element) {
                this.principle = double.parse(element);
              },
      ...
      Padding(
          ...
          child: TextField(
              style: textStyle,
              controller: tec_interest,
              onChanged: (String element) {
                this.interest = double.parse(element);
              },
              keyboardType: TextInputType.number,
              decoration: InputDecoration(
                  labelStyle: textStyle,
          ...
          Expanded(
              child: Padding(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      setState(() {
                        debugPrint("from set State");


                          try{if (form_key.currentState!.validate()) {
                            debugPrint("from if statement claculator button inside if");
                            claculated_intrest =
                            "your calculated Interest is ${(principle * period *
                                interest) / 100}";
                          }}
                          catch(e)
                        {
                          debugPrint("exception is occured $e");
                        }
      ...
      Container(
          ...
          child: Expanded(
              child: Center(
                  child: Text(this.claculated_intrest, style: textStyle))))
    ])));
  }
...
List<DropdownMenuItem<String>> getmenuitems(BuildContext context) {
  ...
  currencies.forEach((element) {
    menuitemlist.add(DropdownMenuItem(
        child: Text(element, style: Theme.of(context).textTheme.subtitle1),
        value: element));
  });
